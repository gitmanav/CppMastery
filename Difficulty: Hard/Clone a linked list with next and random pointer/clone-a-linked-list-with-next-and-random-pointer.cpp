...
class Solution {
  public:
    Node *copyList(Node *head) {
        ...
        Node* temp = head;
// inserting copy nodes in between the original nodes;
        while(temp != NULL){
        ...
        temp = head;
// referencing the random pointers in the copied list
        while(temp != NULL){
...
// at last restoring the orignial list, also referencing the next pointers in the copied list
        Node* res = dnode;
        while(temp != NULL){
            res->next = temp->next;
            res = temp->next;
